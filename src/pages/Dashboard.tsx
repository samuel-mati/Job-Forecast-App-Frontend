...
const Dashboard: React.FC = () => {
  ...
  if (!data) {
    return (
      <div className="p-6 text-sm text-muted">Loading dashboard...</div>
    );
  }
        ...
        <div className="h-[280px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={filteredTrend}>
              ...
              />
              {(selectedCountry === "All" ? countries : [selectedCountry]).map(
                (c, idx) => (
                  <Line
                    key={c}
                    type="monotone"
          ...
          <div className="space-y-3">
            {[...filteredSkills]
              ...
              .map((skill) => (
                <div
                  key={skill.name}
                  className="flex justify-between items-center bg-white/5 px-4 py-2 rounded-md text-sm"
                >
